...
def plot_results(model, X_test, y_test, feature_names, output_path):
    ...
    fig.suptitle('Gradient Boosting Model Results', fontsize=16, fontweight='bold', color=COLOR)
    gs = gridspec.GridSpec(1, 4, figure=fig, wspace=0.4)

    ax0 = fig.add_subplot(gs[0])
    im = ax0.imshow(cm, interpolation='nearest', cmap='Oranges')
...
def main():
    ...
    output_path = os.path.join(OUTPUT_DIR, 'gradient_boosting_result.png')
