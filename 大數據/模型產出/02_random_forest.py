...
axes[1].plot(fpr, tpr, color='green', lw=2, label=f'AUC={metrics["ROC-AUC"]:.4f}')
...
axes[2].step(rec_c, prec_c, color='darkgreen', lw=2, label=f'AP={metrics["Avg Precision"]:.4f}')
...
axes[3].barh(range(len(feat_imp)), feat_imp['Importance'].values, color='mediumseagreen', edgecolor='black')
...
plt.tight_layout()
out_path = os.path.join(OUTPUT_DIR, 'random_forest_result.png')
